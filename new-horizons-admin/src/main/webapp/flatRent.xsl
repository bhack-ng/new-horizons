x<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="/">
		<html>
			...
			<body>
				<h2>Аренда кватиры/ Номер лота
					<xsl:apply-templates select="/flats_rent/offer[1]/id"/>
				</h2>
					...
					<tr>
						...
						<table >
							...
							<tr>
								<td>Адрес:</td>
								<td>
									<xsl:apply-templates select="/flats_rent/offer[1]/address"/>
								</td>
							</tr>
							<tr>
								<td>Метро:</td>
								<td></td>
								<td>
									<xsl:apply-templates select="/flats_rent/offer[1]/metro"/>
								</td>
							</tr>
							<tr>
								<td>Площадь:</td>
								<td>
									<xsl:apply-templates select="/flats_rent/offer[1]/area"/>
								</td>
							</tr>
							<tr>
								<td>Этаж/Этажность:</td>
								<td>
									<xsl:apply-templates select="/flats_rent/offer[1]/floor"/>
								</td>
							</tr>
							<!-- тип здания (панельнй ,кирпичный ...)   -->
							<tr>
									<xsl:apply-templates select="/flats_rent/offer[1]/floor/@type"/>
							</tr>
							...
							<tr>
								...
								<td></td>
								<td>
									<xsl:value-of select="/flats_rent/offer[1]/floor/@seria"/>
								</td>
							</tr>
							<tr>
								<td>Объявление:</td>
								<td></td>
								<td>
									<xsl:value-of select="/flats_rent/offer[1]/note"/>
								</td>
							...
							<tr>
								<td>Цена:</td>
								<td><xsl:apply-templates select="/flats_rent/offer[1]/price"/></td>
							</tr>

							<tr>
								<td>Комиссия агента</td>
								<td></td>
								<td><xsl:value-of select="/flats_rent/offer[1]/com/@agent"/></td>
							</tr>

							<tr>
								<td>Комиссия клиента</td>
								<td></td>
								<td><xsl:value-of select="/flats_rent/offer[1]/com/@client"/></td>
							</tr>
							<tr>
								<td>Кнтрактные телефоны:</td>
								<td></td>
								<td>
									<xsl:apply-templates select="/flats_rent/offer[1]/phone"/>
								</td>
							</tr>
						...
						<table>
							<tr>
								<xsl:apply-templates select="/flats_rent/offer[1]/options"/>
							</tr>

							<tr>
								<td>Комнаты:</td>
								<td>
									<xsl:apply-templates select="/flats_rent/offer[1]/rooms_num"/>
								</td>
							</tr>
						...
						<table >
							...
							<tr>
								...
								<td>
									<xsl:apply-templates select="/flats_rent/offer[1]/photo"/>
								</td>
							</tr>
	...
	<xls:template match="options">
		...
		<tr>
			<td>Наличие мебели:</td>
			<td>
				<xsl:value-of select="document('')/*/my:boolType/entry[@key=$hasMebel]"/>
			</td>
		</tr>
		<tr>
			<td>Наличие мебели на кухне:</td>
			<td>
				<xsl:value-of select="document('')/*/my:boolType/entry[@key=$hasMebelKitchen]"/>
			</td>
		</tr>
		<tr>
			<td>Наличие ванной:</td>
			<td>
				<xsl:value-of select="document('')/*/my:boolType/entry[@key=$hasBath]"/>
			</td>
		</tr>
		<tr>
			<td>Наличие стиральной мишины:</td>
			<td>
				<xsl:value-of select="document('')/*/my:boolType/entry[@key=$hasWM]"/>
			</td>
		</tr>
		<tr>
			<td>Наличие ТВ</td>
			<td>
				<xsl:value-of select="document('')/*/my:boolType/entry[@key=$hasTV]"/>
			</td>
		</tr>
		<tr>
			<td>Можно с детьми</td>
			<td>
				<xsl:value-of select="document('')/*/my:boolType/entry[@key=$children]"/>
			</td>
		</tr>
		<tr>
			<td>Можно с животными</td>
			<td>
				<xsl:value-of select="document('')/*/my:boolType/entry[@key=$pets]"/>
			</td>
		</tr>
		<tr>
			<td>Наличие холодильника</td>
			<td>
				<xsl:value-of select="document('')/*/my:boolType/entry[@key=$hasRefrigerator]"/>
			</td>
		</tr>
		<tr>
	...
	<xsl:variable name="metroId" ><xsl:value-of select="/flats_rent/offer[1]/metro/@id"/></xsl:variable>
	<xsl:variable name="roomCount" ><xsl:value-of select="/flats_rent/offer[1]/rooms_num"/></xsl:variable>
	<xsl:variable name="adminAreaId"><xsl:value-of select="/flats_rent/offer[1]/address/@admin_area"/></xsl:variable>
	<xsl:variable name="buildingType"><xsl:value-of select="/flats_rent/offer[1]/floor/@type"/></xsl:variable>

	<xsl:variable name="hasPhone"><xsl:value-of select="/flats_rent/offer[1]/options/@phone"/></xsl:variable>
	<xsl:variable name="hasMebel"><xsl:value-of select="/flats_rent/offer[1]/options/@mebel"/></xsl:variable>
	<xsl:variable name="hasMebelKitchen"><xsl:value-of select="/flats_rent/offer[1]/options/@mebel_kitchen"/></xsl:variable>
	<xsl:variable name="hasBath"><xsl:value-of select="/flats_rent/offer[1]/options/@bath"/></xsl:variable>
	<xsl:variable name="hasTV"><xsl:value-of select="/flats_rent/offer[1]/options/@tv"/></xsl:variable>
	<xsl:variable name="children"><xsl:value-of select="/flats_rent/offer[1]/options/@kids"/></xsl:variable>
	<xsl:variable name="pets"><xsl:value-of select="/flats_rent/offer[1]/options/@pets"/></xsl:variable>
	<xsl:variable name="hasRefrigerator"><xsl:value-of select="/flats_rent/offer[1]/options/@rfgr"/></xsl:variable>
	<xsl:variable name="hasWM"><xsl:value-of select="/flats_rent/offer[1]/options/@wm"/></xsl:variable>
